//! Network-synced registries for the configuration state.
//!
//! Vela uses a **known-pack passthrough** strategy. In `ClientboundSelectKnownPacks`
//! we advertise the vanilla core data pack (`minecraft:core` / `26.2`). The
//! client confirms it has that exact pack, so in each `ClientboundRegistryData`
//! packet we send only the entry *IDs* with **absent** data — the client fills
//! the real definitions (dimension types, biomes, damage types, …) from its
//! own built-in copy. This is what the vanilla server itself does when client
//! and server share a pack, and it lets us reach the play state without
//! hand-writing any registry NBT (the network NBT codec is deferred).
//!
//! Entry IDs were enumerated from the decompiled 26.2 data pack
//! (`data/minecraft/…`). **Order matters:** the client builds each registry in
...
/// Enabled feature flags advertised in `ClientboundUpdateEnabledFeatures`.
pub const ENABLED_FEATURES: &[&str] = &["minecraft:vanilla"];

/// Network-synced registries, each sent as one `ClientboundRegistryData`
/// packet: `(registry id, &[entry ids])`. Source: `RegistryDataLoader.SYNCHRONIZED_REGISTRIES`.
