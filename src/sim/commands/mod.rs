...
//! commands that take arguments (`/say`, `/me`, `/msg`+`/tell`+`/w`,
//! `/gamemode`) — real argument nodes carrying their parser id and per-parser
//! properties from the [`tree`]'s `Parser` table (a hand-built slice of the
//! `ArgumentTypeInfos` registry). `/tell` and `/w` are `redirect` nodes pointing
//! at `/msg`, exactly as vanilla registers them.
//!
//! The module is split by concern: [`tree`] owns the Brigadier node graph and
//! its wire serialization ([`commands_packet`]); [`handlers`] owns the per-command
//! handlers; [`suggest`] owns the completion engine ([`suggest`]). This file owns
//! the [`COMMANDS`] table and dispatch ([`run`]).
//!
...
// 26.2 rebuilt `/time` around WorldClock/ServerClockManager (`addClockNodes`):
// per-clock set/add/pause/resume/rate and query gametime/absolute/timeline, with
// `commands.time.set.absolute` etc. Vela has no clock manager (only a single
// `WorldTime` resource), so `/time` stays pending rather than advertising a
// pre-26.2 subtree that would emit removed/untranslated keys to the client.
const NEEDS_WORLD_CLOCK: &str = "the world-clock system (ServerClockManager)";
...
static COMMANDS: &[Spec] = &[
    // --- Implemented ---------------------------------------------------------
    Spec::ready("seed", 2, handlers::cmd_seed), // show the world seed
    Spec::ready("list", 0, handlers::cmd_list), // list online players (+ `list uuids`)
    ...
    // --- Chat & messaging ----------------------------------------------------
    Spec::ready("say", 2, handlers::cmd_say),   // broadcast a server message
    Spec::ready("me", 0, handlers::cmd_me),     // broadcast an emote action
    Spec::ready("msg", 0, handlers::cmd_msg),   // private message a player
    Spec::ready("tell", 0, handlers::cmd_msg),  // alias of msg
    Spec::ready("w", 0, handlers::cmd_msg),     // alias of msg
    ...
    // --- Time & progression --------------------------------------------------
    Spec::pending("time", 2, NEEDS_WORLD_CLOCK),        // query/set/add world time
...
mod tests {
    ...
    fn chat_and_messaging_commands_are_ready() {
        for name in ["say", "me", "msg", "tell", "w", "gamemode"] {
            let spec = COMMANDS.iter().find(|c| c.name == name).expect("present");
            assert!(matches!(spec.status, Status::Ready(_)), "/{name} should be Ready");
        }
    }
