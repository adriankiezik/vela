...
- `L` — `[partial]` **World generation**: a believable overworld under `src/world/gen/` (replacing `terrain.rs`). A seamless fbm noise height field (continentalness + hills + detail + a gated mountain term) feeds a temperature/humidity climate field that assigns a handful of biomes (plains, forests, taiga, desert, savanna, jungle, swamp, snowy, ocean, beach, hills). A `SurfaceSystem`-style column rule lays biome-appropriate surface + filler over stone/deepslate, a ragged bedrock floor, sea-level water (y=63) with a frozen ice/snow cap in cold biomes, and a two-field 3D cave carve with a deep lava floor. A deterministic per-chunk decoration pass (`WorldgenRandom`-style `setDecorationSeed`) plants per-biome trees, ore/stone blobs (stone+deepslate variants), grass/ferns, flowers, cacti, and mushrooms. Real per-column biome ids are wired into the chunk biome `PalettedContainer` (network + disk). The chunk baseline is baked into a dense grid once and diffed against disk on load, so player edits round-trip exactly. Deferred: cross-chunk feature bleed (features stay in their origin chunk), aquifers, and structures (`XL`, out of scope)
...
- `L` — `[partial]` **World save/load** (`world/level/storage` — `level.dat`, player data, region files): region files + chunk NBT done (see above). `level.dat` persists seed/spawn/time/game-rules as gzip NBT and reloads the clock + rules on boot (`src/world/storage/level_dat.rs`); per-player `<uuid>.dat` persists position/orientation/held-slot and is restored on rejoin (`src/world/storage/player_dat.rs`). The persisted seed is now fed back into generation on boot (`world::set_seed`) so worlds regenerate reproducibly, and spawn is chosen on a solid, dry surface column (`world::spawn_column`) rather than the fixed origin. Deferred: player inventory round-trip, and per-chunk save-on-eviction (the store never evicts; dirty chunks flush on the 6000-tick autosave + shutdown)
...
- `M` — `[partial]` **Region/chunk coordinate + seed-based RNG utilities** (`util/random`, `valueproviders`): a `java.util.Random`-compatible LCG with `setDecorationSeed` per-chunk seeding drives worldgen decoration (`src/world/gen/rng.rs`, tested against known Java sequences). Xoroshiro (`RandomSource`) and value providers still pending
